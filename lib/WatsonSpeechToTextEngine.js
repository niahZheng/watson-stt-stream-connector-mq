...
class WatsonSpeechToTextEngine extends SpeechToTextEngine {
  ...
  /**
   * Handle discarded audio data
   * This method is called when Genesys sends a 'discarded' message
   * indicating that some audio data was dropped/lost
   * 
   * @param {number} startTime - Start time of discarded audio in seconds
   * @param {number} discardedDuration - Duration of discarded audio in seconds
   */
  handleDiscarded(startTime, discardedDuration) {
    logger.debug({
      event: 'audio_discarded',
      startTime,
      discardedDuration,
      message: 'Audio data was discarded by Genesys'
    });
    // No-op implementation - can be extended if needed
  }
}
